...
export function VoiceRoom({
  ...
  autoJoin?: boolean;
  /** Chamado quando o usuario sai, para o painel encaixado desaparecer. */
  onLeave?: () => void;
}) {
      ...
      {comVideo.length > 0 && (
        <div className="mt-5 space-y-3">
          {emDestaque && focusedUserId && (
            <div
              ref={palcoRef}
              // Sem classe condicional aqui. Antes, o layout de tela cheia
              // vinha de estado React; quando o navegador saia do modo por
              // conta propria (Esc, troca de aba, fim do compartilhamento), o
              // estado e a realidade divergiam e o palco ficava gigante dentro
              // do painel. Agora quem governa e o seletor :fullscreen do CSS,
              // que nunca dessincroniza porque e o proprio navegador.
              className="palco-video relative overflow-hidden rounded border border-amber/50 bg-ink"
            >
              <video
                autoPlay
                playsInline
          ...
          {[eu, ...participants].filter(Boolean).map((userId) => {
            ...
            return (
              <li key={id} className="flex items-center gap-3">
                ...
                <span
                  className={`flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-elevated font-mono text-sm ${
                    falando ? "speaking-ring" : "speaking-ring-idle"
                  }`}
                >
                  {members.get(id)?.user.avatarUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img
                      src={members.get(id)!.user.avatarUrl!}
                      alt=""
        ...
        <div className="mt-5 space-y-3 border-t border-line pt-4">
          ...
          {pushToTalk && (
            <p className="flex flex-wrap items-center gap-2 font-mono text-xs text-muted">
              ...
              >
                {capturandoTecla ? "pressione a tecla…" : "trocar tecla"}
              </button>
              {capturandoTecla && (
                <span className="text-muted">(Esc cancela)</span>
              )}
              {pttHeld && <span className="text-mint">transmitindo</span>}
            </p>
          )}
