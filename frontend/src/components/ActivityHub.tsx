...
/** Convite de servidor esperando resposta. */
...
export function ActivityHub() {
  ...
  const carregar = useCallback(async () => {
    try {
      const [listaServidores, listaConversas, visaoContatos, convitesPendentes] =
        await Promise.all([
          serversApi.list(),
          conversationsApi.list(),
          contactsApi.overview(),
          // Convites de servidor. Estavam sem lugar na interface nova: existiam
          // no backend e nao apareciam em tela nenhuma.
          api
            .get<{ items: ConvitePendente[] }>("/server-invites")
            .then((pagina) => pagina.items)
            .catch(() => [] as ConvitePendente[]),
        ]);
    ...
    <div className="hub">
      ...
      {convites.length > 0 && (
        <section className="hub-section">
          <h2 className="hub-section-title">
            Convites de servidor
            <span className="hub-badge">{convites.length}</span>
          </h2>
          <ul className="hub-list">
            {convites.map((convite) => (
              <li key={convite.id} className="hub-row">
                <span className="hub-avatar">
                  {convite.serverName.slice(0, 2).toUpperCase()}
                </span>
                <span className="hub-row-main">
                  <strong>{convite.serverName}</strong>
                  <span className="hub-row-sub">
                    convite de @{convite.inviterUsername} · expira em{" "}
                ...
                <span className="hub-row-actions">
                  <button
                    ...
                    onClick={() => void responderConvite(convite.id, true)}
                  ...
                  <button
                    type="button"
                    disabled={processando === convite.id}
                    onClick={() => void responderConvite(convite.id, false)}
                    className="hub-btn"
                  >
                    Recusar
                  </button>
          ...
          <ul className="hub-list">
            {pedidos.map((pedido) => (
              <li key={pedido.id} className="hub-row">
                <span className="hub-avatar">
                  {pedido.user.avatarUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={pedido.user.avatarUrl} alt="" />
                  ) : (
          ...
          <ul className="hub-list">
            {conversasRecentes.map((conversa) => (
              <li key={conversa.id} className="hub-row">
                <span className="hub-avatar">
                  {/* Foto do contato. A home era o ultimo lugar que ainda
                      mostrava so as iniciais. */}
                  {conversa.peer.avatarUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={conversa.peer.avatarUrl} alt="" />
                  ) : (
