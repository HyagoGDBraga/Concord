...
export function VoiceStage() {
  ...
  /** Foto de um participante, com as iniciais como reserva. */
  function Retrato({ userId, souEu }: { userId: string; souEu: boolean }) {
    const url = souEu
      ? sessao.members.get(sessao.meuId)?.user.avatarUrl
      : sessao.members.get(userId)?.user.avatarUrl;

    if (url) {
      // eslint-disable-next-line @next/next/no-img-element
      return <img src={url} alt="" />;
    }
    return <>{souEu ? "EU" : sessao.iniciaisDe(userId)}</>;
  }
        ...
        {[eu, ...sessao.participants].map((id) => {
          ...
          return (
            <li key={id} className="voice-person">
              ...
              >
                <Retrato userId={userId} souEu={souEu} />
              </span>
