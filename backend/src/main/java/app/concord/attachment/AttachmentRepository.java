...
public interface AttachmentRepository extends JpaRepository<Attachment, UUID> {
    ...
    @Query("SELECT a FROM Attachment a WHERE a.dmMessageId IN :ids")
    List<Attachment> findByDirectMessageIds(@Param("ids") List<UUID> ids);

    @Query("SELECT a FROM Attachment a WHERE a.channelMessageId IN :ids")
    List<Attachment> findByChannelMessageIds(@Param("ids") List<UUID> ids);
}
