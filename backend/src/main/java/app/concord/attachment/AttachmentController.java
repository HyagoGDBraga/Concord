...
public class AttachmentController {
    ...
    private final app.concord.conversation.ConversationService conversationService;
    ...
    public AttachmentController(AttachmentService service, AttachmentRepository repository,
                                ...
                                app.concord.conversation.ConversationService conversationService) {
    ...
    public AttachmentDtos.Response upload(
        ...
        if (purpose == AttachmentPurpose.MESSAGE) {
            // Exatamente um destino: canal OU conversa. A mesma regra que o
            // banco garante, verificada antes para dar erro legível.
            if ((channelId == null) == (conversationId == null)) {
                throw new ApiException(ErrorCode.VALIDATION_FAILED,
                        "Informe o canal ou a conversa");
            }
            if (channelId != null) {
                requireChannelMember(channelId, user.getId());
            } else {
                conversationService.requireParticipant(conversationId, user.getId());
            }
        }
        return AttachmentDtos.Response.from(
                service.upload(user, file, purpose, channelId, conversationId));
    }
    ...
    public ResponseEntity<Resource> download(
        ...
        if (anexo.getPurpose() == AttachmentPurpose.MESSAGE) {
            // A permissão de baixar segue o destino: participar do canal, ou
            // participar da conversa. Verificada de novo aqui, e não só no
            // envio — uma URL de anexo circula em log e histórico.
            if (anexo.getChannelId() != null) {
                requireChannelMember(anexo.getChannelId(), principal.id());
            } else if (anexo.getConversationId() != null) {
                conversationService.requireParticipant(
                        anexo.getConversationId(), principal.id());
            }
        }
